import{a as b,b as S,n as l}from"./api-0d4e8fa1.js";document.querySelector(".modal");let f=document.querySelector(".close-modal-btn"),y=document.querySelector(".add-favorite-btn"),h=document.querySelector(".give-rating-btn"),d=document.querySelector(".backdrop-modal");document.querySelector(".loader");let r={isFavorite:!1};function I(e){if(!e.target&&!e.target.classList.contains("card-button")&&!e.target.closest(".card-button").classList.contains("card-button"))return;let t;try{t=e.target.closest(".card-button").dataset.id}catch{t=""}t&&(T(),N(t),document.addEventListener("keydown",c),document.querySelector(".backdrop-modal").addEventListener("click",c))}function T(){document.querySelector(".backdrop-modal").classList.remove("visually-hidden"),document.querySelector(".modal").innerHTML='<div class="loader-modal"></div>',document.querySelector(".loader-modal").style.display="block"}function c(e){!e.target.classList.contains("backdrop-modal")&&!e.target.classList.contains("give-rating-btn")&&!e.target.closest(".close-modal-btn")&&e.code!=="Escape"&&!d.classList.contains("visually-hidden")||(d.classList.add("visually-hidden"),document.removeEventListener("keydown",c),d.removeEventListener("click",c))}function M(e){Object.hasOwn(r,"isFavorite")&&r.isFavorite?(A(e),r.isFavorite=!1,l("success","The exercise has been removed from your favorites list")):(_(),r.isFavorite=!0,l("success","The exercise has been added to your favorites list")),L(r)}function _(e){let t=JSON.parse(localStorage.getItem("favorites"))||[];t.push(r),localStorage.setItem("favorites",JSON.stringify(t))}function A(e){const t=e.target.dataset.id;let i=(JSON.parse(localStorage.getItem("favorites"))||[]).filter(s=>s._id!==t);localStorage.setItem("favorites",JSON.stringify(i))}function B(e){if(e){let t,a=e;return t=(JSON.parse(localStorage.getItem("favorites"))||[]).filter(s=>s._id===a),r.isFavorite=!!t.length,t}}function N(e){B(e),j(e)}function L(e){document.querySelector(".modal").innerHTML=R(e),f=document.querySelector(".close-modal-btn"),f.addEventListener("click",c),y=document.querySelector(".add-favorite-btn"),y.addEventListener("click",M),h=document.querySelector(".give-rating-btn"),h.addEventListener("click",P)}function P(e){document.querySelector(".backdrop-modal").classList.add("visually-hidden"),x(document.querySelector(".backdrop-modal"),e)}function j(e){e&&b.get(`${S}/${e}`,{}).then(t=>{r.isFavorite?(r=t,r.isFavorite=!0):(r=t,r.isFavorite=!1),L(t)}).catch(t=>l("error",`API error: ${t}`)).finally(()=>{try{document.querySelector(".loader-modal").style.display="none"}catch{}})}function u(e){return e.charAt(0).toUpperCase()+e.slice(1)}function R({_id:e,bodyPart:t,equipment:a,gifUrl:i,name:s,target:n,description:E,rating:g,burnedCalories:F,time:q,popularity:$,isFavorite:C}){let p=5,v=Number(g.toFixed());p-=v;let O='<r class="star-1"/>'.repeat(v).concat('<r class="star-0"/>'.repeat(p));return`<div class="modal-description-container">
    ...
    </div>`}const G=Object.freeze(Object.defineProperty({__proto__:null,closeModalHandler:c,openModalHandler:I},Symbol.toStringTag,{value:"Module"}));let m,k;const o={icons:document.querySelectorAll(".rating-star-icon"),form:document.querySelector(".rating-form"),stars:document.querySelector(".rating-star-list"),backdrop:document.querySelector(".backdrop-rating")};o.icons.forEach((e,t)=>{e.addEventListener("mouseover",()=>{for(let a=0;a<t;a++)o.icons[a].classList.add("hovered")}),e.addEventListener("mouseout",()=>{for(let a=0;a<t;a++)o.icons[a].classList.remove("hovered")})});o.form.addEventListener("submit",J);o.stars.addEventListener("click",H);o.backdrop.addEventListener("click",w);document.addEventListener("keydown",w);function H(e){if(e.target.nodeName!=="INPUT")return;const t=e.currentTarget.previousElementSibling;t.textContent=parseFloat(e.target.value).toFixed(1);const a=Array.from(e.currentTarget.querySelectorAll(".rating-star-icon")),i=a.indexOf(e.target.nextElementSibling);a.forEach((s,n)=>{s.classList.toggle("selected",n<=i)})}function J(e){e.preventDefault();const t=parseInt(o.form.querySelector(".rating-value").textContent),a=e.currentTarget.elements.email.value,i=e.currentTarget.elements.comment.value,s={rate:t,email:a,review:i};b.patch(`${S}/${k}/rating`,JSON.stringify(s)).then(n=>{l("success","Rating has been updated!")}).catch(n=>{n.message.response.status===409?l("warning",`Warning: ${n.message.response.data.message}`):l("error",`API error: ${n.message.response.data.message}`)})}function w(e){!e.target.classList.contains("backdrop-rating")&&!e.target.classList.contains("close")&&!e.target.closest(".close")&&e.code!=="Escape"||(m&&m.classList.remove("visually-hidden"),o.backdrop.classList.add("visually-hidden"),document.addEventListener("keydown",c))}function x(e,t){m=e,k=t.target.dataset.id,o.backdrop.classList.remove("visually-hidden"),document.removeEventListener("keydown",c)}const U=Object.freeze(Object.defineProperty({__proto__:null,openRatingModal:x},Symbol.toStringTag,{value:"Module"}));export{I as a,G as e,x as o,U as r};
//# sourceMappingURL=rating_modal-78eeb366.js.map
